 ...
 */
const { promiseRequest, promisePost, globalAgent } = require('./async-https.js');
...
class Destiny2API {
    constructor(config = {}) {
        ...
        this.key = config.key;
        this.userAgent = config.userAgent || globalAgent;
        this.oauthConfig = {
    ...
    /**
    * Gets information of a Bungie.net Account
    * @param {number} id The requested Bungie.net membership id.
    */
    getBungieNetUserById(id) {
        this.options.path = `/Platform/User/GetBungieNetUserById/${id}/`;
        this.options.method = 'GET';
        return promiseRequest(this.options).then(res => formatJson(res));
    }

    /**
    * Gets Destiny memberships given a bungie name
    * @param {string} displayName bungie name ex: Hello#1234
    * @param {number} membershipType enum of values for specifying platform
    */
    searchDestinyPlayerByBungieName(displayName, displayNameCode) {
        this.options.path = '/Platform/Destiny2/SearchDestinyPlayerByBungieName/All/'
        this.options.method = 'POST';
        return promisePost(this.options, { displayName: displayName, displayNameCode: displayNameCode })
            .then(res => formatJson(res));
    }

    /**
    * Gets all linked accounts and their membershipId's
    * @param {string} membershipType type of membership enum
    * @param {number} membershipId account id (platform specific)
    */
    getMembershipDataById(membershipType, membershipId) {
        this.options.path = `/Platform/User/GetMembershipsById/${membershipId}/${membershipType}/`
        this.options.method = 'GET';
        return promiseRequest(this.options).then(res => formatJson(res));
    }
    ...
    */
    ...
    /**
     * Deprecated and unsupported. Bungie removed this endpoint.
     * See: https://github.com/Bungie-net/api/issues/1922
    ...
    searchDestinyEntities(type, searchTerm, page) {
        const message = 'searchDestinyEntities is unsupported because Bungie deprecated ' +
